...
<body>
    ...
    <footer class="center footer">
        <h6 class="info"><b>2021 © Made by Chetan Kumar</b></h6>
...
if (isset($_POST['submit'])) {
    ...
    // constraint to check zero values
        ...
        $sql3 = "UPDATE clients set c_balance=$newbalance where c_id=$from";
        mysqli_query($conn, $sql3);

        // adding amount to reciever's account
        $newbalance = $sql2['c_balance'] + $amount;
        $sql4 = "UPDATE clients set c_balance=$newbalance where c_id=$to";
        $query3 = mysqli_query($conn, $sql4);
		
		if($query3){
		//get name
		
		$sql_history_name = "SELECT c_name from clients where c_id=$from ";
		$query_history_name = mysqli_query($conn, $sql_history_name);
		$sql_history_name = mysqli_fetch_array($query_history_name);
		$SENDER_NAME = $sql_history_name['c_name'];
		
		$sql_history_name1 = "SELECT c_name from clients where c_id=$to ";
		$query_history_name1 = mysqli_query($conn, $sql_history_name1);
		$sql_history_name1 = mysqli_fetch_array($query_history_name1);
		$RECEIVER_NAME = $sql_history_name1['c_name'];
		
		
		$sql_history = "SELECT (max(sr_no)+1 ) s_no from transaction_history ";
		$query_history = mysqli_query($conn, $sql_history);
		$sql_history = mysqli_fetch_array($query_history);
		$s_no = $sql_history['s_no'];
        $sql3 = "INSERT INTO transaction_history(`sr_no`, `sender`, `receiver`, `balance`, `date_time`) VALUES ('$s_no','$SENDER_NAME','$RECEIVER_NAME','$amount',NOW())";
